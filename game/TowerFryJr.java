...
public class TowerFryJr extends Tower {
	...
	/**
	 * Constructor for the fry Jr. stationary tower. Takes in position and the game state
	 * as parameters. Initializes the range.
	 * 
	 * @param state - the game state
	 * @param x - x coordinate
	 * @param y - y coordinate
	 */
	public TowerFryJr(GameState state, int x, int y) {
		...
		// Set first call to be true so that the spawn time resets correctly when the tower shoots
		// lightning. (remove this to create an exploit where tower will rapid fire for at an enemy for
		// a split second if tower is placed while an enemy is within range)
		firstCall = true;
	...
	public void draw(Graphics g, GameView view) {
		...
		// Draw itself at the current position.
		view.drawCenteredImage(g, "JrFry.png", x, y, 1.0);
	...
	/**
	 * Allows the tower to shoot brain waves.
	 */
	private void shootBrainWave() {
		...
		// Fire one shot every second.
		if (state.getTime() > spawnTime) {
			state.addGameObject(new EffectBrainWave(state, this, x, y));
			spawnTime += 1.0;
		}
	}
}
